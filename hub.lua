...
local Window = Rayfield:CreateWindow({
   Name = "BestBBallScript",
   LoadingTitle = "Powered by the Rayfield Interface Suite",
   LoadingSubtitle = "by Sirius, QTFeline",
   ConfigurationSaving = {
      Enabled = true,
...
local BladeTab = Window:CreateTab("BladeBallExclusives", 4483362458) -- Title, Image

local BloxTab = Window:CreateTab("BloxFruitExclusives", 4483362458) -- Title, Image

local Tab = Window:CreateTab("Misc", 4483362458) -- Title, Image

local Rivals = Window:CreateTab("Rivals", 4483362458) -- Title, Image

local Toggle = BladeTab:CreateToggle({
   Name = "Auto Parry",
   CurrentValue = false,
...
local Toggle = PlayerTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        local function onRenderStep()
            if noClipEnabled then
                ...
                if userInputService:IsKeyDown(Enum.KeyCode.Space) then
                    moveDirection = Vector3.new(0, 1, 0)  -- Move up
                elseif userInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
                    moveDirection = Vector3.new(0, -1, 0) -- Move down
                end
        ...
        -- Listen for the key press to toggle no-clip
        ...
        -- Connect the RenderStepped event for continuous movement
 ...
 local Toggle = PlayerTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        -- Function to start and stop the fake lag script
        ...
        UserInputService.InputBegan:Connect(function(input)
            if input.KeyCode == Enum.KeyCode.L then -- Change 'L' to any key you want to toggle
                toggleLag()
            end
        end)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            ...
            local moving = (lastPosition - currentPosition).Magnitude > 0.05 -- Threshold for movement detection
        
            if moving then
                if not isMoving then
                    isMoving = true
                    startTime = tick() -- Reset start time when we first start moving
                elseif tick() - startTime > lagTime then
                    -- Rubberband logic (move player slightly backward)
                    local backwardPosition = currentPosition - (rootPart.CFrame.LookVector * 3) -- Move 3 studs backward
                    rootPart.CFrame = CFrame.new(backwardPosition)
                    startTime = tick() -- Reset timer after rubberbanding
                end
            else
                isMoving = false
                startTime = tick() -- Reset start time when not moving
            end
 ...
 local Toggle = Tab:CreateToggle({
    Name = "Fly",
    CurrentValue = false,
    Flag = "Toggl51", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(Value)
        local player = game.Players.LocalPlayer
        local character = player.Character or player.CharacterAdded:Wait()
        local userInputService = game:GetService("UserInputService")
        ...
        local Button = Tab:CreateButton({
            ...
            Callback = function()
                ...
                -- Store the player's current position in a shared variable
                _G.savedCoordinates = rootPart.Position
                print("Coordinates set: " .. tostring(_G.savedCoordinates))
            end,
        })
        ...
        local Button = Tab:CreateButton({
            ...
            Callback = function()
                ...
                -- Check if coordinates were set and teleport the player to them
                if _G.savedCoordinates then
                    rootPart.CFrame = CFrame.new(_G.savedCoordinates)
                    print("Teleported to: " .. tostring(_G.savedCoordinates))
                else
                    warn("No coordinates set!")
                end
            end,
        })
...
RunButton.Position = UDim2.new(0.5, -50, 0, 60)
RunButton.BackgroundColor3 = Color3.fromRGB(0, 170, 0)
RunButton.Text = "Run"
RunButton.Parent = Frame

MinimizeButton.Size = UDim2.new(0, 30, 0, 30)
MinimizeButton.Position = UDim2.new(0.95, -15, 0, 5)
...
local function teleportToPlayer(playerName)
    ...
    if player then
        ...
        if character and character:FindFirstChild("HumanoidRootPart") then
            ...
            local offset = Vector3.new(-5, 0, 0) -- Offset to teleport behind the playerAAAAAA
            for i = 1, 100 do
                wait(0.01) -- Wait for 0.01 seconds (100 times per second)
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = rootPart.CFrame + offset
            end
        end
    else
...
MinimizeButton.MouseButton1Click:Connect(function()
    ...
    Frame.Position = IsMinimized and UDim2.new(0.5, -150, 0.5, -100) or UDim2.new(0.5, -150, 0.5, -75)
    TextBox.Visible = not IsMinimized
    RunButton.Visible = not IsMinimized
end)
...
Frame.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement and dragging then
        ...
        Frame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
		 ...
		 local Button = Rivals:CreateButton({
			Name = "Rivals Hub",
			Callback = function()
				local UserInputService = game:GetService("UserInputService")
				...
				-- Flag to control the aimbot
				local v37 = false
				
				-- Function to start aimbot when LT is pressed
				...
				-- Connect the LT button to start and stop aimbot
		 ...
		 local Button = PlayerTab:CreateButton({
			...
			Callback = function()
				...
				-- Create a GUI button for teleporting
				local ScreenGui = Instance.new("ScreenGui")
				...
				TeleportButton.Position = UDim2.new(1, -210, 1, -60) -- Position at bottom right
				...
				local function enableFreeCam()
					...
					Character:Move(Vector3.new(0, 0, 0)) -- Stop character movement
				end
				...
				-- Function to teleport character to camera
				...
				RunService.RenderStepped:Connect(function()
					if freeCamEnabled then
						...
						Camera.CFrame = Camera.CFrame * CFrame.Angles(0, -math.rad(mouseDelta.X * rotationSpeed), 0)
						Camera.CFrame = Camera.CFrame * CFrame.Angles(-math.rad(mouseDelta.Y * rotationSpeed), 0, 0)
						...
						if moveDirection.Magnitude > 0 then
							moveDirection = moveDirection.Unit * 10 * RunService.RenderStepped:Wait() -- Speed of movement
							Camera.CFrame = Camera.CFrame + moveDirection
						end
					end
				end)
				
				-- Bind teleport function to button
				TeleportButton.MouseButton1Click:Connect(function()
					teleportToCamera()
				end)
				
				-- Enable free camera when the player presses the 'E' key
				UserInputService.InputBegan:Connect(function(input, gameProcessed)
				...
				-- Disable mouse cursor visibility when free cam is active
